...
class _ResultSectionState extends State<ResultSection> {
  ...
  Future<void> _loadOriginalImage() async {
    try {
      // AssetBundle을 사용하여 이미지 리소스 로드
      final ByteData data = await DefaultAssetBundle.of(
        context,
      ).load(AppConstants.resultImagePath);
      setState(() {
        _originalImageBytes = data.buffer.asUint8List();
      });
    } catch (e) {
      debugPrint('원본 이미지 로드 중 오류: $e');
      // 오류 발생 시 UI에 알림
      _showMessage('원본 이미지를 로드할 수 없습니다.');
    }
  }
                ...
                child: Center(
                  child: ClipRRect(
                    ...
                    child: Container(
                      ...
                      child: Image.asset(
                        AppConstants.resultImagePath,
                        fit: BoxFit.fitHeight, // 상하에 맞게 이미지 조정
                        width: double.infinity,
